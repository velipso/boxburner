...
class SurfaceBuilder {
    ...
    build(thickness) {
        return new Surface(thickness, this.border.close().build(), this.holes.map(({ offset, db }) => ({
            offset,
...
class PlainBox extends GeneratorBase {
    ...
    generate(settings, { width, depth, height, holeDistance, play, thicknessPlay }) {
        const { thickness } = settings;
...
// SPDX-License-Identifier: 0BSD
//
const allGenerators = [new Rectangle(), new PlainBox()];
...
export { AlongIntersection, BoxJoint, ButtJoint, DocumentBase, DocumentSVG, DrawBuilder, EdgeBase, GeneratorBase, MortiseAndTenonJoint, PlainBox, Rectangle, SettingsTypeDef, Surface, SurfaceBuilder, allEdges, allEdgesTypeDef, allGenerators, copyVec2, eps, expandPathByKerf, exportDocument, forwardVec2, linesIntersect };
