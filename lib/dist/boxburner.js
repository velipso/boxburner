...
class BoxCardstock extends GeneratorBase {
    ...
    schema() {
        const tabsProperties = (allowFlip) => ({
            properties: {
                kind: {
                    type: 'string',
                    metadata: {
                    ...
                    properties: {
                        ...
                        insertPadding: {
                            ...
                            metadata: {
                                ...
                                description: 'How tight the fit around the tabs are after insertion',
                            },
                        },
                        tabWidth: {
                            type: 'float64',
                        ...
                        tabExtension: {
                            ...
                            metadata: {
                                default: 2,
                                title: 'Tab Extension',
                                description: 'How much the tab sticks out (multiples of material thickness)',
                            },
                        },
                        tabCount: {
                            type: 'float64',
    ...
    generate(settings, { thickness, kerf, width, depth, height, scorePadding, wrapConnection, topCover, bottomCover, }) {
        const th = typeof thickness === 'number' ? thickness : settings.defaultThickness;
        const kr = typeof kerf === 'number' ? kerf : settings.defaultKerf;
        const r1x = 0;
        const r1y = 0;
        const r1w = width;
        const r1h = height;
        const depthPad = topCover.kind === 'tuck' || bottomCover.kind === 'tuck' ? th : 0;
        const r2x = r1x + r1w;
        const r2y = 0;
        ...
        const addTuckTab = (shape, notchWidth, x, y, w, d) => {
            border = border
                .combine(shape()
                .rect(x, y, w, -d)
                .combine(shape()
                ...
                .lineTo(x + w - notchWidth * 0.2 - (d - notchWidth) * 0.1, y - d)
                .lineTo(x + w, y - d)
                .closePath())
                .difference())
                .union();
        ...
        const addTuckFlap = (shape, notchWidth, notchHeight, x, y, w, d) => {
            ...
            if (ny <= 0) {
                ...
                cuts = cuts
                    ...
                    .bezierCurveTo(x + notchWidth, y, x + notchWidth, y, x + notchWidth, y + ny)
                    .lineTo(x + notchWidth, y + notchHeight)
                    // top right
                    .moveTo(x + w, y)
                    .lineTo(x + w - notchWidth + ny, y)
                    .bezierCurveTo(x + w - notchWidth, y, x + w - notchWidth, y, x + w - notchWidth, y + ny)
        ...
        const addTuck = (isTop, { notchWidth, notchHeight, }) => {
            const shape = () => isTop
                ? polybool.shape()
            ...
            addTuckTab(() => shape().scale(-1, 1), notchWidth, -r4x - r4w, r4y, r4w - th, depth);
            scores = scores
                .combine(shape()
        ...
        const addReceiverTab = (w, h, tx, ty, ang) => {
            border = border
                .combine(polybool
                .shape()
                .translate(tx, ty)
        ...
        const addInsertTabs = ({ receiverWidth, insertPadding, tabWidth, tabHeight, tabInnerCut, tabCount, tabExtension, }, height, t1x, t1y, ang1, t2x, t2y, ang2, additionalSpace) => {
            if (additionalSpace) {
                border = border
                    .combine(polybool
                    .shape()
            ...
            for (let i = 0; i < tabCount; i++) {
                ...
                border = border
                    ...
                    .bezierCurveTo(ext + tabWidth, y + ty, ext + tabWidth, y + ty, ext + tabWidth, y + ty + cy)
                    .lineTo(ext + tabWidth, y)
                    .closePath())
                    .difference()
                    .combine(polybool
                    ...
                    .bezierCurveTo(ext + tabWidth, y + h - ty, ext + tabWidth, y + h - ty, ext + tabWidth, y + h - ty - cy)
                    .lineTo(ext + tabWidth, y + h)
                    .closePath())
                    .difference())
                    .union();
                if (receiverWidth > 0) {
                    ...
                    cuts = cuts
                        ...
                        .bezierCurveTo(-th, y + h - tic, -th, y + h - tic, 0, y + h - tic))
                        .union();
                }
            }
        };
        switch (topCover.kind) {
            ...
            case 'tabs':
                ...
                scores = scores
                    ...
                    .union();
                addInsertTabs(topCover.params, r1w, topCover.params.flip ? r1x + r1w + r2x + r2w : r1x + r1w, -th, 90, topCover.params.flip ? 0 : r3x, -r4w - th * 2, 270, true);
                break;
            case 'tuck':
        ...
        switch (bottomCover.kind) {
            ...
            case 'tabs':
                ...
                scores = scores
                    ...
                    .union();
                addInsertTabs(bottomCover.params, r1w, bottomCover.params.flip ? r3x : 0, height + th, 270, bottomCover.params.flip ? r1w : r4x - th, height + r4w + th * 2, 90, true);
                break;
            case 'tuck':
        ...
        switch (wrapConnection.kind) {
            ...
            case 'tabs':
                addInsertTabs(wrapConnection.params, height, r1x, 0, 0, r4x + r4w, 0, 0, false);
                break;
            case 'tuck':
                if (topCover.kind !== 'tabs' || bottomCover.kind !== 'tabs') {
                    throw new Error('Top and bottom must be tabs');
                }
                addTuckTab(topCover.params.flip
                    ? () => polybool
                        .shape()
                    ...
                    : () => polybool
                        ...
                        .rotateDeg(90)
                        .scale(-1, 1), wrapConnection.params.notchWidth, 0, 0, depth, depth);
                addTuckTab(bottomCover.params.flip
                    ? () => polybool
                        .shape()
                    ...
                    : () => polybool
                        ...
                        .rotateDeg(90), wrapConnection.params.notchWidth, 0, 0, depth, depth);
                addTuckFlap(() => polybool
                    .shape()
                    .translate(r4x + r4w, 0)
                    .rotateDeg(90), wrapConnection.params.notchWidth, wrapConnection.params.notchHeight, 0, 0, height, depth);
                break;
            default:
...
export { ApplyKerf, BoxCardstock, BoxJoint, BoxNested, BoxPlain, ButtJoint, DocumentBase, DocumentSVG, EdgeBase, GeneratorBase, JointBase, KerfTester, LegEdge, MortiseAndTenonJoint, Rectangle, SettingsTypeDef, Shape, Surface, allEdges, allEdgesTypeDef, allGenerators, allJoints, boxJointFingerSpacer, edgeListTypeDef, exportDocument, geo, polybool };
